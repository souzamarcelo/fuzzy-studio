 ...
 *
 */
public class FunctionBlockAdaptada extends FclObject implements Iterable<RuleBlock>, Comparable<FunctionBlock> {
	...
	public FunctionBlockAdaptada(FIS fis) {
		this.fis = fis;
	...
	public String fclTree(Tree tree) {
		...
		for (int childNum = 0; childNum < tree.getChildCount(); childNum++) {
			...
			if (firstChild) name = leaveName;
				...
				RuleBlock ruleBlock = new RuleBlock(copiaFB());
	...
	private MembershipFunction fclTreeFuzzifyTermCosine(Tree tree) {
		if (debug) Gpr.debug("Tree: " + tree.toStringTree());
		Value net_max = new Value(tree.getChild(0), copiaFB());
		Value offset = new Value(tree.getChild(1), copiaFB());
	...
	private MembershipFunction fclTreeFuzzifyTermDifferenceSigmoidal(Tree tree) {
		if (debug) Gpr.debug("Tree: " + tree.toStringTree());
		Value a1 = new Value(tree.getChild(0), copiaFB());
		Value c1 = new Value(tree.getChild(1), copiaFB());
		Value a2 = new Value(tree.getChild(2), copiaFB());
		Value c2 = new Value(tree.getChild(3), copiaFB());
	...
	private MembershipFunction fclTreeFuzzifyTermFunction(Tree tree) {
		if (debug) Gpr.debug("Tree: " + tree.toStringTree());
		return new MembershipFunctionFuncion(copiaFB(), tree.getChild(0));
	}
	...
	private MembershipFunction fclTreeFuzzifyTermGauss(Tree tree) {
		if (debug) Gpr.debug("Tree: " + tree.toStringTree());
		Tree child = tree.getChild(0);
		Value mean = new Value(child, copiaFB());
		Value stdev = new Value(tree.getChild(1), copiaFB());
	...
	private MembershipFunction fclTreeFuzzifyTermGenBell(Tree tree) {
		if (debug) Gpr.debug("Tree: " + tree.toStringTree());
		Tree child = tree.getChild(0);
		Value a = new Value(child, copiaFB());
		Value b = new Value(tree.getChild(1), copiaFB());
		Value mean = new Value(tree.getChild(2), copiaFB());
	...
	private MembershipFunction fclTreeFuzzifyTermPieceWiseLinear(Tree tree) {
		...
		for (int childNum = 1; childNum < tree.getChildCount(); childNum++) {
			...
			if (leaveName.equalsIgnoreCase("POINT")) {
				x[childNum - 1] = new Value(child.getChild(0), copiaFB()); // Parse and add each point
				y[childNum - 1] = new Value(child.getChild(1), copiaFB());
	...
	private MembershipFunction fclTreeFuzzifyTermSigmoidal(Tree tree) {
		if (debug) Gpr.debug("Tree: " + tree.toStringTree());
		Value gain = new Value(tree.getChild(0), copiaFB());
		Value t0 = new Value(tree.getChild(1), copiaFB());
	...
	private MembershipFunction fclTreeFuzzifyTermSingleton(Tree tree) {
		if (debug) Gpr.debug("Tree: " + tree.toStringTree());
		Value singleTonValueX = new Value(tree, copiaFB());
	...
	private MembershipFunction fclTreeFuzzifyTermSingletonsPoints(Tree tree, int numberOfPoints) {
		...
		for (int childNum = 0; childNum < tree.getChildCount(); childNum++) {
			...
			if (leaveName.equalsIgnoreCase("(")) {
				x[childNum] = new Value(child.getChild(0), copiaFB()); // Parse and add each point
				y[childNum] = new Value(child.getChild(1), copiaFB());
	...
	private MembershipFunction fclTreeFuzzifyTermTrapetzoidal(Tree tree) {
		if (debug) Gpr.debug("Tree: " + tree.toStringTree());
		Value min = new Value(tree.getChild(0), copiaFB());
		Value midLow = new Value(tree.getChild(1), copiaFB());
		Value midHigh = new Value(tree.getChild(2), copiaFB());
		Value max = new Value(tree.getChild(3), copiaFB());
	...
	private MembershipFunction fclTreeFuzzifyTermTriangular(Tree tree) {
		if (debug) Gpr.debug("Tree: " + tree.toStringTree());
		Value min = new Value(tree.getChild(0), copiaFB());
		Value mid = new Value(tree.getChild(1), copiaFB());
		Value max = new Value(tree.getChild(2), copiaFB());
	...
	public boolean varibleExists(String variableName) {
        ...
        public FunctionBlock copiaFB(){
            FunctionBlock fb = new FunctionBlock(this.fis);
            fb.setName(this.getName());
            fb.setRuleBlocks(this.getRuleBlocks());
            fb.setVariables(this.getVariables());
